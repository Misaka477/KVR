...
| Kernel | 加速 | 用途 | 状态 |
|:------|:---:|:-----|:-----|
| `score_kernel` | 10.5x | 检索搜索（unpack + deq + RoPE + dot） | ⚠️ 修复了 Q 取法未对齐 int4 interleave 的 bug，但 d=64 下 triton-windows 有不可复现的数值差异（cos、K_pre、K_post 全部匹配但 score 不一致，疑似社区版编译器 bug）。默认使用 Python 路径（NIAH 100%）。|
| `fused_attn_kernel` | 1.7x | 级联注意力 | ✅ 可用 |

### 当前搜索路径

`compute_all_scores` 使用 Python 路径：

```
_deq_k (unpack int4 → deq) → _rotary → dot → scores
```

Triton `score_kernel` 的 Q interleave 修复已确认正确（d=4 下 diff=0），但在 d=64 的 triton-windows 社区版上有数值差异（原始 PyTorch `@` 用 TF32，Triton 用 fp32，但关闭 TF32 后仍有差异）。目前 Python 路径的 10.5x 比预期低（Python loop overhead），但 NIAH 100%。

### 已验证但不使用的算子

| Kernel | 结论 | 原因 |
|:------|:----|:------|
| `group_softmax_kernel` | 正确但慢 | Triton 无法调 tensor core 做 batch matmul，cuBLAS einsum 是此场景最优解 |

### Bug 修复记录：score_kernel Q 取法

**根因：** int4 打包将 K 的维度按奇偶交错排列（hi=偶维, lo=奇维），但 score_kernel 用连续前半/后半取 Q（q[:HALF], q[HALF:]），导致 Q 和 K 的维度配对错位。

**修复：** 将 Q 取法改为与 int4 相同的交错排列——`q_even = q[0::2]`, `q_odd = q[1::2]`。

**验证：** d=4 下全部 diff=0、cos/sin 索引正确、K_pre/K_post 一致。d=64 下 solo 验证也匹配，但全量 kernel 调用时有 ~0.03 的累积差异（原因未明，可能是 triton-windows 社区版的编译器优化问题）。
...
| 配置 | 通过率 | 说明 |
|:----|:------:|:-----|
| Python 路径（当前默认） | **100%** | 搜索 + 注意力全程 Python |
| Triton score_kernel 路径 | 83% | d=64 下有 ~0.03 浮点差异，导致排序变化。修复已应用（Q interleave），差异源自社区版编译器 |
...
7. **Triton score_kernel 有 triton-windows 社区版兼容性问题**：Q interleave bug 已修复且 d=4 下验证正确，但 d=64 下有不可复现的数值差异，疑似社区版编译器优化问题。Python 搜索路径已足够（10.5x 搜索加速暂不可用）。
